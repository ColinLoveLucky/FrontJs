...
import { queryWhiteListByConditionAsync, exportWhiteListByIdsAsync, deleteWhiteListByIdsAsync, batchOperateWhiteListByCheckpPointAsync } from '../../services/whiteapi';
import { actionType, responseStatus, whiteListStatusType } from '../../utils/constObject';
...
export default {
  ...
  effects: {
    ...
    *loadData({payload},{call,put}){
      ...
      }else if(action == actionType.DELETE || action == actionType.ACTIVATE){
        const ids = payload.ids;
        let whiteListStatus = null;
        if(action == actionType.DELETE){
          whiteListStatus = whiteListStatusType.INACTIVE;
        }else if(action == actionType.ACTIVATE){
          whiteListStatus = whiteListStatusType.ACTIVE;
        }else{
          // Do Nothing
        }
        let deleteResponse = yield call(deleteWhiteListByIdsAsync, whiteListStatus, ids);
        console.log(deleteResponse);
        if(deleteResponse != null && deleteResponse.data != null){
          if(deleteResponse.data.result == responseStatus.SUCCESS){
            let queryResponse = yield call(queryWhiteListByConditionAsync, searchModel);
            yield put({
              type: 'getWhilteListByCondition',
              payload: queryResponse,
          ...
          }else if(deleteResponse.data.result == responseStatus.ERROR){
            message.error(deleteResponse.data.errorMsg);
          }
        }else{
    ...
    *batchOperateByCheckpPoint({payload},{call,put}){
      ...
      const response = yield call(batchOperateWhiteListByCheckpPointAsync, checkPoint, operateType, file);
      yield put({
        type: 'batchOperate',
        payload: response,
      });
